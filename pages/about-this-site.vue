...
const { data: pages } = await useAsyncData(
  'about-this-site-page-variants',
  () => queryCollection('pages').where('slug', '=', 'about-this-site').all(),
)
...
useHead({
  title: () => page.value?.title ?? 'About this site',
})
</script>
  ...
  <div class="w-screen flex flex-col lg:flex-row">
    ...
    >
      <div class="w-full max-w-2xl px-4 pt-8 pb-16 sm:px-0">
        ...
        >
          <template #empty>
            <UEmpty icon="i-lucide-file-text" title="Empty site page" />
          </template>
        </ContentRenderer>
        ...
        <UEmpty
          ...
          description="content/about-this-site.md is missing."
        />
      </div>
    </main>
  </div>
