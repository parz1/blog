...
在二分类模型中，它的概率含义来自一个更具体的约定。模型把线性分数 $z$ 看作类别 $1$ 的对数几率：

$$
z = \log\frac{p}{1-p}
$$

把这条式子中的 $p$ 解出来，结果正是 $p=\sigma(z)$。因此，Sigmoid 在逻辑回归中承担的是“从对数几率还原概率”的工作。$z=0$ 对应 $p=0.5$；$z$ 为正时类别 $1$ 更占优势；$z$ 为负时类别 $0$ 更占优势。

::MathFigure{preset="sigmoid"}
::

也可以把它理解成一个“平滑的概率开关”：输入 $z$ 越大，输出越接近 $1$；输入越小，输出越接近 $0$；当 $z=0$ 时，输出恰好是 $0.5$。
...
这里的 $p$ 是当前参数产生的预测。训练前若所有参数都为 $0$，模型会暂时对每条样本输出 $0.5$；参数经过数据调整后，预测也会随之改变。Sigmoid 给出了概率模型的形式，概率是否可靠仍取决于数据、训练和校准。
